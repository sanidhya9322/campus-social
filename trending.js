...
onAuthStateChanged(auth, async (user) => {
    if (!user) {
        ...
        // Only load trending posts AFTER confirming the user is authenticated
        loadTrendingPosts();
    }
});
    ...
    onSnapshot(trendingQuery, (snapshot) => {
        ...
        snapshot.forEach((docSnapshot) => {
            ...
            likeBtn.addEventListener('click', async function() {
                if (!currentUser) return; // Final fallback check
            ...
            commentBtn.addEventListener('click', async function() {
                ...
                if (commentText.trim() === "" || !userProfile || !currentUser) return;
                
                await updateDoc(doc(db, "campus_posts", postId), {
                    comments: arrayUnion({
                        text: commentText,
                        authorName: userProfile.fullName,
                        userId: currentUser.uid // Stores the user ID securely as requested
                    })
                });
                commentInput.value = "";
            });
